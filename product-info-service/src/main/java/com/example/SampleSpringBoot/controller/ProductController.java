...
import com.example.SampleSpringBoot.dto.response.*;
...
public class ProductController {
    ...
    @CrossOrigin
    public ResponseEntity<ProductViewResponse> viewProduct(@PathVariable int id){
    ...
    @CrossOrigin
    public ResponseEntity<ProductUpdateResponse> updateProduct(@PathVariable int id, @Validated @RequestBody ProductUpdateRequest request) throws Exception{
        Product product = modelMapper.map(request, Product.class);
        product.setId(id);
    ...
    @GetMapping("${app.endpoint.productRetrieve}")
    @CrossOrigin
    public ResponseEntity<List<ProductRetrieveResponse>> viewProduct(){
        List<Product> productList = productService.retrieveProduct();
        List<ProductRetrieveResponse> productRetrieveResponses = productList.stream().map(product -> modelMapper.map(product, ProductRetrieveResponse.class)).collect(Collectors.toList());
        return new ResponseEntity<>(productRetrieveResponses, HttpStatus.OK);
    }
}
